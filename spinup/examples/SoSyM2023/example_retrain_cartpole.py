...
def plot_trajectories(trajectories1, trajectories2, save_name):
    ...
    for i in range(len(trajectories1)-1):
        ...
        axis[0].plot(x1, np.linspace(0, len(x1)-1, len(x1)), 'g--')
        axis[1].plot(theta1, np.linspace(0, len(theta1)-1, len(theta1)), 'g--')
        x2, _, theta2, _ = zip(*trajectories2[i])
        axis[0].plot(x2, np.linspace(0, len(x2)-1, len(x2)), 'b')
        axis[1].plot(theta2, np.linspace(0, len(theta2)-1, len(theta2)), 'b')
    x1, _, theta1, _ = zip(*trajectories1[-1])
    axis[0].plot(x1, np.linspace(0, len(x1)-1, len(x1)), 'g--', label='baseline')
    axis[1].plot(theta1, np.linspace(0, len(theta1)-1, len(theta1)), 'g--', label='baseline')
    x2, _, theta2, _ = zip(*trajectories2[-1])
    axis[0].plot(x2, np.linspace(0, len(x2)-1, len(x2)), 'b', label='retrained')
    axis[1].plot(theta2, np.linspace(0, len(theta2)-1, len(theta2)), 'b', label='retrained')
    """"""
    ...
    if "spinup" in cwd:
        if "examples" in cwd:
            if "SoSyM2023" in cwd:
                ...
                log_directory = "./SoSyM2023/logs/ex_retrain_cartpole/"
                fig_directory = "./SoSyM2023/logs/figures/ex_retrain_cartpole/"
        else:
            log_directory = "./examples/SoSyM2023/logs/ex_retrain_cartpole/"
            fig_directory = "./examples/SoSyM2023/logs/figures/ex_retrain_cartpole/"
    else:
