...
if __name__ == "__main__":
    ...
    parser.add_argument('--retrain', help="TODO",
                        action="store_true")
    ...
    if "spinup" in cwd:
        if "examples" in cwd:
            if "sefm2022" in cwd:
                log_directory = "./logs/ex_bipedal_walker/"
                fig_directory = "./logs/figures/ex_bipedal_walker/"
            else:
                log_directory = "./sefm2022/logs/ex_bipedal_walker/"
                fig_directory = "./sefm2022/logs/figures/ex_bipedal_walker/"
        else:
            log_directory = "./examples/sefm2022/logs/ex_bipedal_walker/"
            fig_directory = "./examples/sefm2022/logs/figures/ex_bipedal_walker/"
    else:
        log_directory = "/tmp/logs/ppo/ex_bipedal_walker/"
        fig_directory = "/tmp/logs/ppo/ex_bipedal_walker/figures/ex_bipedal_walker/"
    stl_env_config = args['config_path'] + "ex_bipedal_walker.yaml"
    stl_env_config_eval = args['config_path'] + "ex_bipedal_walker_eval.yaml"
    ...
    if args['baseline']:
        for i in range(len(random_seeds)):
            env_fn = partial(gym.make, 'BipedalWalker-v3')
            test_env_fn = partial(gym.make, 'BipedalWalker-v3')
            ...
            original_env = gym.make('BipedalWalker-v3')
            stl_env = stlgym.make(stl_env_config_eval)
            evaluate_policy_in_2_environments(env1=original_env, env2=stl_env, get_action=get_action, log_dest=log_dest, max_ep_len=max_ep_len, num_episodes=num_evals)
