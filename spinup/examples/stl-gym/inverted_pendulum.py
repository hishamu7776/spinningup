...
if __name__ == "__main__":
    ...
    if args['baseline']:
        for i in range(len(random_seeds)):
            env_fn = partial(gym.make, 'Pendulum-v1', reward_style=1)
            test_env_fn = partial(STLGym, 'SafePendulum-v0', reward_style=2)
            ...
            ppo(env_fn, test_env_fn, alt_env_fn=None, ac_kwargs=ac_kwargs, mod=0, seed=random_seeds[i],
                ...
                target_kl=target_kl, logger_kwargs=logger_kwargs, save_freq=save_freq)
            ...
            rta_off_env = gym.make('Pendulum-v1')
            rta_on_env = gym.make('SafePendulum-v0')
            evaluate_policy(rta_on_env, rta_off_env, get_action, log_dest, max_ep_len=200, num_episodes=num_evals)

    # Training with STLGym reward function
    if args['stl']:
        for i in range(len(random_seeds)):
            env_fn = partial(gym.make, 'Pendulum-v1', reward_style=1)
            test_env_fn = partial(gym.make, 'SafePendulum-v0', reward_style=2)
            alt_env_fn = partial(gym.make, 'Pendulum-v1')
            log_dest = log_directory + "stl/rand_seed_" + str(random_seeds[i])
            logger_kwargs = dict(output_dir=log_dest, exp_name=exp1_name)
            print(f"Training PPO stl, random seed: {random_seeds[i]}...")
            ppo(env_fn, test_env_fn, alt_env_fn, ac_kwargs=ac_kwargs, mod=0, seed=random_seeds[i],
                steps_per_epoch=steps_per_epoch, epochs=epochs, gamma=gamma, clip_ratio=clip_ratio, pi_lr=pi_lr,
                vf_lr=vf_lr, train_pi_iters=train_pi_iters, train_v_iters=train_v_iters, lam=lam,
                num_test_episodes=num_test_episodes, max_ep_len=max_ep_len,
                target_kl=target_kl, logger_kwargs=logger_kwargs, save_freq=save_freq)
    ...
    if args['re-stl']:
        for i in range(len(random_seeds)):
            ...
            test_env_fn = partial(gym.make, 'SafePendulum-v0', reward_style=2)
            alt_env_fn = partial(gym.make, 'Pendulum-v1')
            log_dest = log_directory + "re-stl/rand_seed_" + str(random_seeds[i])
            logger_kwargs = dict(output_dir=log_dest, exp_name=exp1_name)
